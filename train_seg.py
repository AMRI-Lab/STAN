...
from loss import CEDice, DiceLoss, CELDice, FLDice, EdgeFLDice, UncertaintyLoss
...
def main(res):
    ...
    train_cut_point = int(whole_data_length * 0.8)
    train_set = files[:train_cut_point]
    val_set = files[train_cut_point: ]
    ...
    for epoch in range(opt.epochs):
        ...
        train_loss, train_dice = train(train_loader=train_loader, model=model, seg_crit=seg_crit, optimizer=optimizer, epoch=epoch)
        valid_loss, valid_dice = validate( valid_loader=valid_loader, model=model, seg_crit=seg_crit)        
        ...
        if valid_metric > best_metric:
            ...
            print('=======>   Best at epoch %d, valid dice %f\n' % (epoch, best_metric))
    ...
    os.system('echo " ==== TRAIN DICE mtc:{:.5f}" >> {}'.format(train_dice, res))
    print('Epo - Mtc')
...
def train(train_loader, model, seg_crit, optimizer,  epoch):
    ...
    for i, (img, mask, _, age, gender, _) in enumerate(train_loader):
        ...
        if opt.ismixup:
            ...
            index = torch.randperm(input.size(0)).cuda() # 将0~n-1（包括0和n-1）随机打乱后获得的数字序列
        ...
        model.train()
        optimizer.zero_grad()
        segout, gloout = model(input, gender)
        age_loss = 0
        ...
        if i % opt.print_freq == 0:
            print('Epoch: [{0} / {1}]   [step {2}/{3}]\t'
                  ...
                  (epoch, opt.epochs, i, len(train_loader), SegLoss=SegLoss, loss=Loss, Dice=Dice))

        loss.backward()
        nn.utils.clip_grad_norm_(model.parameters(), max_norm=5, norm_type=2)
        optimizer.step()

    return Loss.avg, Dice.avg

def validate(valid_loader, model, seg_crit):
    ...
    with torch.no_grad():
        for _, (img, mask, _, age, gender, _) in enumerate(valid_loader):
            ...
            segout, gloout = model(img, gender)
            age_loss = 0
            seg_loss = seg_crit(segout, mask.long())
            loss = opt.lam2 * age_loss + opt.lam1 * seg_loss
            dice = dice_coeff(segout.detach(), mask.long(), num_classes = segout.size(1)).cpu().data.numpy()
        ...
        print('Valid: [steps {0}]\t'
            ...
            'Loss {loss.val:.4f} ({loss.avg:.4f})\t'
            'Dice {Dice.val:.3f} ({Dice.avg:.3f})\t'.format
            (len(valid_loader), SegLoss=SegLoss, loss=Loss, Dice=Dice))
